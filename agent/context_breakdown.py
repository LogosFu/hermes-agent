...
def _memory_blocks(agent: Any) -> Tuple[str, str, str]:
    ...
    try:
        ...
        if store is not None and getattr(agent, "_project_memory_enabled", False):
            project_block = store.format_for_system_prompt("project") or ""
    except Exception:
        pass
    return memory_block, user_block, project_block
...
def compute_session_context_breakdown(agent: Any, messages: Optional[List[dict]] = None) -> Dict[str, Any]:
    ...
    memory_block, user_block, project_block = _memory_blocks(agent)
    system_prompt_text = _join(
        _strip_blocks(stable, skills_index), _strip_blocks(parts.get("volatile", "") or "", memory_block, user_block, project_block)
    )
    ...
    tokens_by_id = {
        ...
        "memory": _chars_to_tokens(_join(memory_block, user_block, project_block)),
